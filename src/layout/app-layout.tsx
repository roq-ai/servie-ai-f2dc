...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<FacilityInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/facilities` : null),
    () => getFacilities({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Facility',
      path: isTenantUser ? `/facilities/view/${data?.[0]?.id}` : '/facilities',
      entity: 'facility',
      service: AccessServiceEnum.PROJECT,
      icon: FiHome,
    },
    {
      name: 'Automated Call Rule',
      path: '/automated-call-rules',
      entity: 'automated_call_rule',
      service: AccessServiceEnum.PROJECT,
      icon: FiPhoneCall,
    },
    {
      name: 'Automated Call Status',
      path: '/automated-call-statuses',
      entity: 'automated_call_status',
      service: AccessServiceEnum.PROJECT,
      icon: FiPhoneMissed,
    },
    { name: 'Resident', path: '/residents', entity: 'resident', service: AccessServiceEnum.PROJECT, icon: FiUsers },
